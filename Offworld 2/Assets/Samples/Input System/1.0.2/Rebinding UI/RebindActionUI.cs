...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        /// <summary>
        /// Toggle component that handles the inversion of inputs. Can be <c>null</c> in which
        /// case the inversion is completely ignored.
        /// </summary>
        public Toggle invertToggle
        {
            get => m_InvertToggle;
            set => m_InvertToggle = value;
        }
        ...
        public void InvertAxis(){
            ...
            if(action.expectedControlType.ToLower() == "axis"){
                if(!m_InvertToggle.isOn){
                    action.ApplyBindingOverride(new InputBinding{path = action.bindings[bindingIndex].path,
                                                                overridePath = action.bindings[bindingIndex].overridePath,
                                                                overrideProcessors = "clamp(min=-1,max=1)"});
                }else{
                    action.ApplyBindingOverride(new InputBinding{path = action.bindings[bindingIndex].path,
                                                                overridePath = action.bindings[bindingIndex].overridePath,
                                                                overrideProcessors = "clamp(min=-1,max=1), invert"});
                }
            }
        }
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                .WithMagnitudeHavingToBeGreaterThan(0.9f)
                    .OnCancel(
                        operation =>
                        {
        ...
        [Tooltip("Optional UI that handles the inversion of the input")]
        [SerializeField]
